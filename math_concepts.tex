...
\usepackage{Sweave}
\begin{document}
\input{math_concepts-concordance}
...
\begin{dfn}
\boxed{A^c} \quad The \emph{complement} of $A$ in $U$ is $A^c = U \setminus A$.
\end{dfn}
...
\begin{dfn}
\boxed{A \times B} \quad The \emph{Cartesian product} of $A$ and $B$ is $A \times B = \{(a,b): a \in A and b \in B \} $. This can be thought of as a way to "multiply" sets. The product of A and B creates a new set with each element forming an ordered pair (a,b).
      \begin{itemize}
            \item Here is an example: If $A = \{1, 3, 5 \}$ and $B = \{\omega, \Omega \} $ then \\
                  $A \times B = \{(1, \omega), (1, \Omega), (3, \omega), (3, \Omega), (5, \omega), (5, \Omega) \} $. \\
                  These elements can be generated via table too:
                  \begin{tabular}{c | c | c | c}
                        & 1 & 3 & 5 \\
                        \hline
                        $\omega$ & $(1, \omega)$ & $(3, \omega)$ & $(5, \omega)$ \\
                        $\Omega$ & $(1, \Omega)$ & $(3, \Omega)$ & $(5, \Omega)$ \\
                  \end{tabular} \\
                  If $\omega = \frac{1}{2}$ and $\Omega = \frac{3}{2}$ then we can have Cartesian coordinates as well: \\
